...
3. **Given** a user views the header below the `sm:` breakpoint, **When** the navigation no longer fits comfortably, **Then** the navigation is available through a menu control on the left side of the header.
...
**Independent Test**: Can be fully tested by using the GitHub link, switching themes, and confirming the login button is visible as a placeholder on desktop viewports and below the `sm:` breakpoint.
...
- Header content must not overlap, wrap awkwardly, or obscure the main content below the `sm:` breakpoint.
...
- **FR-003**: The header MUST display an A.S.C.A. logo on the left side using the exact text "A.S.C.A."; a logo image is not required for this feature.
...
- **FR-006**: On viewports below the `sm:` breakpoint, the system MUST collapse the central navigation into a left-side menu control while keeping both navigation destinations available.
...
- **FR-015**: The layout MUST keep header controls readable, operable, and non-overlapping across desktop viewports and viewports below the `sm:` breakpoint.
...
- **SC-003**: In usability checks across desktop viewports and viewports below the `sm:` breakpoint, 95% of attempts to identify the current page from the header succeed without additional instruction.
- **SC-004**: Header controls remain visible and non-overlapping on desktop viewports and viewports below the `sm:` breakpoint, with no critical control hidden from users.
...
- The layout is expected to support both desktop users and users on viewports below the `sm:` breakpoint.
